...
public final class ReciprocalArraySum {
    ...
    protected static double parManyTaskArraySum(final double[] input,
        ...
        // create list of tasks to process based on numTasks
        ...
        // process first n-1 tasks in parallel
        for(int i = 0; i < tasks.size()-1; i++) {
            tasks.get(i).fork();
        }

        // compute the remaining task
        tasks.get(tasks.size()-1).compute();

        // join the parallel tasks
        for(int i = 0; i < tasks.size()-1; i++) {
            tasks.get(i).join();
        }

        // add up results from all computed tasks
